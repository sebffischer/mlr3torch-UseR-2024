...
* GitHub: [https://github.com/mlr-org/mlr3](https://github.com/mlr-org/mlr3)
...
* Define a simple regression tree `Learner` from the {rpart} package
...
:::
:::{.column width="50%"}
* `mlr3pipelines`^[M. Binder and B. Bischl et al, JMLR, 2021] allows assemble new `Learner`s by connecting `PipeOp`s in a `Graph`, e.g. for preprocessing
* `PipeOp`s are created via `po()`, and combined using `%>>%`
<br>
...
- Data Types:
    ...
    - Well suited for multi-modal data
...
::: {.column width="50%"}
* On the right, we construct and resample a multi layer perceptron with one hidden layer of size 20 and `ReLU` activation
...
design = benchmark_grid(tsk_mtcars, list(lrn_mlp, lrn_tree), rsmp_cv)
...
:::{.columns}
:::{.column width="50%"}
* The images of the MNIST task are represented as `lazy_tensor`s, which wrap a `torch::dataset` that does not necessarily have to store the data in-memory
* We have to reshape the MNIST images to work with our MLP that expects 2d inputs
* The preprocessing of `lazy_tensor`s happens lazily by internally building up a preprocessing `Graph` that
* We can cmbine the flattening step with our previous graph into a new `GraphLearner`
:::
:::{.column width="50%"}
```{r}
tsk_mnist = tsk("mnist")
...
* Here, we optimize the latent dimension of the network and the learning rate of the optimizer
