...
::: {.column width="50%"}
* On the right, we construct and resample a multi layer perceptron with one hidden layer of size 10 and `ReLU` activation
...
dat = data.table(
  ...
  xray = as_lazy_tensor(torch_tensor(array(runif(100 * 3 * 64 * 64), dim = c(100, 3, 64, 64)))),
...
:::{.columns}
::: {.column width="50%"}
* For example, we might have some medical data about patients, as well as x-ray images
* Because `lazy_tensor`s are just vectors, they can be conveniently stored alongside tabular data 
* `mlr3torch` supports building neural networks with multiple inputs, e.g. each operating on different features
:::
::: {.column width="50%"}
...
* Here, we optimize the latent dimension of the network and the learning rate of the optimizer using Bayesian Optimization from `mlr3mbo`.
